...
weather_noNA %>% 
  ...
  group_by(Location) %>% 
  summarise(rain_count = n()) %>% 
  arrange(desc(rain_count))
```
...
```{r}
# remove eval=FALSE
weather_noNA$RainTomorrow <- factor(weather_noNA$RainTomorrow, levels = c("No", "Yes"))
glimpse(weather_noNA)
...
```{r}
unique(weather_Portland$RainTomorrow)
```

```{r}

model_port <- logistic_reg() %>%
  set_engine("glm") %>%
...
```{r}
model_pred <- predict(model_port, weather_Portland, type = "prob") %>%
  bind_cols(weather_Portland) 

ggplot(mapping = aes(x=.pred_No, y=.pred_Yes), data = model_pred) +
  geom_point()
...
*The prodicted probablilities only take two values*

- Are there any missing values? Why?

```{r}
colSums(is.na(model_pred))
```

*These will be values where RainToday was NA.*
...
```{r}
port_split <- initial_split(weather_Portland, prop = 0.80)
# Create data frames for the two sets:
train_data <- training(port_split)
test_data  <- testing(port_split)
```
